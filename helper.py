...
creator_mnemonic = "list same claim daring myself session nice husband welcome make cinnamon aunt enemy video broom online that gold spatial hockey shadow rubber auto ability name"
user_mnemonic = "tragic allow decrease era garment shine replace elite net copy repair whale ramp myth gift inch glow depth habit idle bacon among reason ability duty"
...
if __name__ == "__main__":
    c_private_key = get_private_key_from_mnemonic(creator_mnemonic)
    u_private_key = get_private_key_from_mnemonic(user_mnemonic)
    to = "GD64YIY3TWGDMCNPP553DZPPR6LDUSFQOIJVFDPPXWEG3FVOJCCDBBHU5A"
    crt = account.address_from_private_key(c_private_key)
    usr = account.address_from_private_key(u_private_key)
    ...
    ind = 0
    ind_usr = 0
    account_info = algod_client.account_info(crt)
    account_info_usr = algod_client.account_info(usr)
    apps_created = account_info["created-apps"]
    apps_opted_in = account_info_usr["apps-local-state"]

    print("indx: ", ind)
    for app in apps_created:
        print("app: ", app["id"])
        txns.append(ApplicationDeleteTxn(sender=crt, sp=params, index=app["id"]))
        apps[ind] = app["id"]
        ind += 1

    for app in apps_opted_in:
        print("app_clr: ", app["id"])
        txns.append(ApplicationClearStateTxn(sender=usr, sp=params, index=app["id"]))
        apps_usr[ind_usr] = app["id"]
        ind_usr += 1

    print("indx: ", ind)
    txns.append(PaymentTxn(sender=crt,sp=params,receiver=to,amt=0,close_remainder_to=to))
    txns.append(PaymentTxn(sender=usr,sp=params,receiver=to,amt=0,close_remainder_to=to))
    print("txns.length: ", len(txns))
    
    group_id = transaction.calculate_group_id(txns)

    for i in apps:
        print("i: ", i)
        txns[i].group = group_id
        # sign transactions
        stxn.append(txns[i].sign(c_private_key))

    for i in apps_usr:
        print("i_usr: ", i)
        txns[i+ind].group = group_id
        # sign transactions
        stxn.append(txns[i+ind].sign(u_private_key))

    print("indx: ", ind)
    print("indx_usr: ", ind_usr)
    txns[ind+ind_usr].group = group_id
    txns[ind+ind_usr+1].group = group_id

    stxn.append(txns[ind+ind_usr].sign(c_private_key))
    stxn.append(txns[ind+ind_usr+1].sign(u_private_key))
    ...
    print("atomic transaction groupid: ",txid)
    #main()
